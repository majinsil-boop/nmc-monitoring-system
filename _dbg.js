const _els={};function makeEl(id){return{id,textContent:'',innerHTML:'',className:'',value:'',style:{transition:'',opacity:''},classList:{toggle:()=>{},add:()=>{},remove:()=>{},contains:()=>false},remove:()=>{}}}
const document={getElementById:(id)=>(_els[id]=_els[id]||makeEl(id)),querySelectorAll:()=>[],createElement:()=>makeEl('_'),body:{appendChild:()=>{}}};
const window={};const fetch=()=>Promise.resolve({ok:true,json:()=>Promise.resolve({})});const alert=m=>console.log('ALERT:',m);const setTimeout=(fn,ms)=>{};
...
console.log('assembly:',store.assembly.length,'news:',store.news.length);const ml=_els['mainList'];console.log('mainList:',ml?ml.innerHTML.length:0,'chars');if(ml)process.stdout.write('FIRST200:'+ml.innerHTML.substring(0,200)+'
');
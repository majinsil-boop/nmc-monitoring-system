...
±¹È¸ÀÇ¾ÈÁ¤º¸½Ã½ºÅÛ ÀÚµ¿ °Ë»ö ½ºÅ©¸³Æ®

Å°¿öµå: ÀÀ±ÞÀÇ·á, 119±¸Á¶±¸±Þ¹ý, °ø°øº¸°ÇÀÇ·á¹ý, ÀÇ·á¹ý, ´ë¾È(º¸°Çº¹ÁöÀ§¿øÀå), ÇÊ¼öÀÇ·á¹ý, ÀÇ·á, ÀÀ±Þ
...
try:
    ...
    print("[°æ°í] openpyxl ¹Ì¼³Ä¡ - history_db.xlsx ±â´É ºñÈ°¼ºÈ­. pip install openpyxl")
...
def _summarize_with_claude(raw_text: str, bill_name: str = "") -> str:
    ...
    Claude Code CLI(claude -p)·Î ÀÇ¾È ³»¿ëÀ» 2-3¹®Àå ÇÑ±¹¾î ¿ä¾à.
    ...
    prompt = (
        ...
        "´ÙÀ½Àº ±¹È¸ ÀÇ¾ÈÀÇ Á¦¾ÈÀÌÀ¯ ¹× ÁÖ¿ä³»¿ëÀÔ´Ï´Ù.\n"

        "ÇÙ½É °³Á¤ »çÀ¯¿Í ÁÖ¿ä º¯°æ ³»¿ëÀ» 2~3¹®ÀåÀ¸·Î °£°áÇÏ°Ô ÇÑ±¹¾î·Î ¿ä¾àÇØ ÁÖ¼¼¿ä.\n"

        "Àü¹® ¿ë¾î´Â ±×´ë·Î À¯ÁöÇÏµÇ ´©±¸³ª ÀÌÇØÇÏ±â ½±°Ô ½á ÁÖ¼¼¿ä.\n"

        "¿ä¾à¹®¸¸ Ãâ·ÂÇÏ°í ´Ù¸¥ ¼³¸íÀº ºÙÀÌÁö ¸¶¼¼¿ä.\n\n"
    ...
    try:
        ...
        if result.stderr:
            ...
            print(f"  [Claude ¿ä¾à ¿À·ù] {result.stderr[:120]}")

        return raw_text

    except subprocess.TimeoutExpired:

        print("  [Claude ¿ä¾à Å¸ÀÓ¾Æ¿ô]")

        return raw_text

    except Exception as e:

        print(f"  [Claude ¿ä¾à ½ÇÆÐ] {e}")
...
# ÀÇ·á¹ý¡¤119±¸Á¶±¸±Þ¡¤ÇÊ¼öÀÇ·á¡¤´ë¾È¡¤ÀÇ·á: ÀÀ±Þ °ü·Ã ³»¿ë Æ÷ÇÔ ÇÊ¼ö

_NARROW_KEYWORDS: set[str] = {"ÀÇ·á¹ý", "119±¸Á¶±¸±Þ", "ÇÊ¼öÀÇ·á", "´ë¾È", "ÀÇ·á"}

_NARROW_CONTENT_FILTER: list[str] = [

    "ÀÀ±Þ½Ç", "ÀÀ±ÞÀÇ·á", "ÁßÁõ¿Ü»ó", "±¸±Þ", "ÀÀ±ÞÃ³Ä¡",

]

# ÀÀ±Þ Å°¿öµå: ¹ý¾È¸í ¶Ç´Â ¿ä¾à¿¡ "ÀÀ±Þ" Æ÷ÇÔ ÇÊ¼ö

_EMERGENCY_ONLY_KEYWORDS: set[str] = {"ÀÀ±Þ"}



# ¿ì¼±¼øÀ§ ÀÇ¿ø ¸ñ·Ï ? ¹ßÀÇÀÚ¿¡ Æ÷ÇÔ ½Ã ¡Ú Ç¥½Ã ¹× ÃÖ»ó´Ü ¹èÄ¡

PRIORITY_MEMBERS: set[str] = {"ÀÌ¼öÁø", "±è¹Ì¾Ö", "±èÀ±", "±è¼±¹Î", "ÇÑÁö¾Æ", "ÀÌÁÖ¿µ"}
...
SEARCH_CONCURRENCY = 4   # µ¿½Ã Å°¿öµå °Ë»ö ¼ö

DETAIL_CONCURRENCY = 5   # µ¿½Ã »ó¼¼ ÆäÀÌÁö ¼öÁý ¼ö



_DONE_KEYWORDS = ("Æó±â", "°øÆ÷", "ÀÌ¼Û", "°¡°á", "ºÎ°á", "Ã¶È¸")





# ¦¡¦¡ ÀÌ·Â DB (history_db.xlsx) ¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡



def _load_history() -> dict[str, dict]:

    """history_db.xlsx¿¡¼­ ÀÌÀü ¼öÁý ÀÌ·ÂÀ» ·Îµå. {bill_no: row_dict}"""
...
def _save_history(bills: list[dict], history: dict[str, dict]):
    ...
    """ÇöÀç ¼öÁý °á°ú¸¦ history_db.xlsx¿¡ ´©Àû ÀúÀåÇÑ´Ù."""
    ...
    try:
        ...
        changed_fill  = PatternFill("solid", fgColor="FFE699")  # »óÅÂº¯°æ Çà: ³ë¶õ»ö

        priority_fill = PatternFill("solid", fgColor="C6EFCE")  # ¿ì¼±ÀÇ¿ø Çà: ¿¬µÎ»ö
        ...
        print(f"[ÀÌ·Â DB] {HISTORY_DB} ÀúÀå ¿Ï·á (ÃÑ {len(history)}°Ç)")

    except Exception as e:

        print(f"[ÀÌ·Â DB ÀúÀå ¿À·ù] {e}")
...
_CORE_FILTER_KW: list[str] = ["ÀÀ±Þ½Ç", "ÀÀ±ÞÀÇ·á", "ÁßÁõ¿Ü»ó", "ÁßÁõÀÀ±Þ"]


def _passes_content_filter(bill: dict) -> bool:
    """
    ÇÕ°Ý ±âÁØ:
      1) ÀÇ¾È¸íÀÌ 'ÀÀ±ÞÀÇ·á'·Î ½ÃÀÛ ¡æ ÇÕ°Ý
      2) ÀÇ¾È¸í+¿ä¾à¿¡ ÀÀ±Þ½Ç¡¤ÀÀ±ÞÀÇ·á¡¤ÁßÁõ¿Ü»ó¡¤ÁßÁõÀÀ±Þ Áß ÇÏ³ª Æ÷ÇÔ ¡æ ÇÕ°Ý
      3) ¼Ò°üÀ§¿øÈ¸°¡ º¸°Çº¹ÁöÀ§¿øÈ¸ AND ÀÔ¹ý¿¹°í ±â°£ Áß ¡æ ÇÕ°Ý
    """
    ...
    # º¸°Çº¹ÁöÀ§¿øÈ¸ ¼Ò°ü + ÀÔ¹ý¿¹°í ±â°£ ÁßÀÎ ÀÇ¾È
    if "º¸°Çº¹Áö" in committee and _is_notice_active(bill.get("legislative_notice", "")):
        print(f"    ¡æ [º¸°Çº¹ÁöÀ§] ÀÔ¹ý¿¹°í Áß ÀÇ¾È Æ÷ÇÔ: {name[:40]}")
        return True

    if _is_notice_active(bill.get("legislative_notice", "")):
        print(f"    ¡æ [ÀÔ¹ý¿¹°í Áß] Æ÷ÇÔ: {name[:40]}")
        return True
...
def _is_priority(bill: dict) -> bool:
    ...
    """¹ßÀÇÀÚ Áß ¿ì¼±¼øÀ§ ÀÇ¿øÀÌ Æ÷ÇÔµÇ¾î ÀÖÀ¸¸é True."""
...
def _clean_bill_name(title_attr: str) -> str:
    ...
    return re.sub(r"\s*\([^)]*ÀÇ¾È[^)]*\)\s*$", "", title_attr).strip()
...
def _apply_status_change_label(bill: dict, history: dict[str, dict]):
    ...
    """ÀÌÀü »óÅÂ¿Í ºñ±³ÇØ »óÅÂº¯°æ ·¹ÀÌºíÀ» bill¿¡ Ãß°¡ÇÑ´Ù."""
    ...
    if prev_status and prev_status != curr_status:
        ...
        bill["status_change_label"] = f"[»óÅÂº¯°æ: {prev_status} ¡æ {curr_status}]"

        bill["status_changed"] = True

        print(f"    ¡Ú »óÅÂº¯°æ °¨Áö: {bill.get('bill_name','')[:30]} "

              f"| {prev_status} ¡æ {curr_status}")
...
async def _fetch_bill_detail(page, url: str) -> tuple[str, str, str, str]:
    """(summary, status_date, notice, committee) ¹ÝÈ¯."""
    ...
    try:
        ...
        result = await page.evaluate("""
            ...
            () => {
                ...
                const STOP_RE = /½É»çÁøÇà|ÀÔ¹ýÁøÇà|ÀÇ¾È¹øÈ£|Á¦¾ÈÀÚ|\\n{3,}/;



                for (const keyword of ['Á¦¾ÈÀÌÀ¯', 'ÁÖ¿ä³»¿ë']) {
                ...
                const progIdx = body.search(/½É»çÁøÇà|ÀÔ¹ýÁøÇà|½É»ç°æ°ú/);
                ...
                for (const el of candidates) {
                    ...
                    if (txt.includes('ÀÔ¹ý¿¹°í') && /\\d{4}.*~.*\\d{4}/.test(txt) && txt.length < 120) {
                ...
                if (!notice) {
                    ...
                    const m = body.match(/ÀÔ¹ý¿¹°í[Áß]?\\s*\\(\\s*\\d{4}[-.\\d]+\\s*~\\s*\\d{4}[-.\\d]+\\s*\\)/);
                ...
                // Æú¹é1: tr ÀüÃ¼ ÅØ½ºÆ®¿¡¼­ ÀÎÁ¢ ´Ü¾î ÃßÃâ
                if (!committee) {
                    for (const tr of document.querySelectorAll('table tr')) {
                        const txt = (tr.innerText || '').replace(/\\s+/g, ' ').trim();
                        if (txt.includes('¼Ò°üÀ§¿øÈ¸')) {
                            const m = txt.match(/¼Ò°üÀ§¿øÈ¸\\s*([°¡-ÆR\\s]+À§¿øÈ¸)/);
                            if (m) { committee = m[1].trim().slice(0, 50); break; }
                        }
                    }
                }
                // Æú¹é2: body ÀüÃ¼ ÅØ½ºÆ® (ÁÙ¹Ù²Þ Æ÷ÇÔ)
                if (!committee) {
                    const m = body.match(/¼Ò°üÀ§¿øÈ¸[\\s\\S]{0,10}?([°¡-ÆR]+À§¿øÈ¸)/);
...
async def _dismiss_popup(page):
    """popup-zone__wrap µî ÆË¾÷/¿À¹ö·¹ÀÌ¸¦ JS·Î °­Á¦ Á¦°Å."""
    try:
        await page.evaluate("""
            () => {
...
async def search_bills(page, keyword: str) -> list[dict]:
    ...
    try:
        ...
        print("  ¸ÞÀÎ ÆäÀÌÁö Å¸ÀÓ¾Æ¿ô, Àç½Ãµµ...")
    ...
    if not search_input:
        ...
        print("  °Ë»ö ÀÔ·Â¶õ ¾øÀ½, ´ëÃ¼ ¼¿·ºÅÍ ½Ãµµ...")

        search_input = await page.query_selector("input[placeholder*='ÀÇ¾È¸í']")

    if not search_input:

        print("  °Ë»ö ÀÔ·Â¶õÀ» Ã£À» ¼ö ¾ø½À´Ï´Ù.")
    ...
    print(f"  ÆäÀÌÁö 1: {len(page_results)}°Ç")
    ...
    while True:
        ...
        if not next_link:
            ...
            next_group = await page.query_selector("a.btn-next-group, a[title*='´ÙÀ½ ÆäÀÌÁö']")
        ...
        print(f"  ÆäÀÌÁö {page_num}: {len(page_results)}°Ç")
    ...
    print(f"  ¼öÁý ¿Ï·á: {len(all_results)}°Ç")
...
async def main():
    ...
    print(f"[ÀÌ·Â DB] ±âÁ¸ ÀÌ·Â {len(history)}°Ç ·Îµå")
    ...
    async with async_playwright() as p:
        ...
        print(f"\n[º´·Ä °Ë»ö] {len(KEYWORDS)}°³ Å°¿öµå µ¿½Ã °Ë»ö (ÃÖ´ë {SEARCH_CONCURRENCY}°³)")
        ...
        # ¦¡¦¡ 2. ¸ñ·Ï ÆäÀÌÁö 1Â÷ ÇÊÅÍ: ³¯Â¥ Á¶°Ç Åë°úÇÑ °Í¸¸ »ó¼¼ ¹æ¹® ¦¡¦¡¦¡¦¡¦¡¦¡

        to_detail =  [
            r for r in all_results
            if _is_recent(r)
            or "ÀÇ·á" in r.get("bill_name", "")
            or "ÀÀ±Þ" in r.get("bill_name", "")
            or "º¸°Ç" in r.get("bill_name", "")
]

        # history_db¿¡ ÀÔ¹ý¿¹°í ±â°£ ÁßÀ¸·Î ±â·ÏµÈ ÀÇ¾È ¡æ ¸ñ·Ï¿¡ ¾ø¾îµµ Àç¹æ¹®
        ...
        for hist_no, hist_row in history.items():
            ...
            if not url:
                continue
            # to_detail¿¡ º¹¿ø (¸ñ·Ï ÆäÀÌÁö ±âº»°ªÀ¸·Î)
            to_detail.append({
                "bill_no":       hist_no,
        ...
        print(f"\n[1Â÷ ÇÊÅÍ] {len(all_results)}°Ç ¡æ {len(to_detail)}°Ç »ó¼¼ Á¶È¸ "

              f"({skipped}°Ç ¹ßÀÇ¡¤ÀÇ°áÀÏ {RECENT_DAYS}ÀÏ ÀÌ³» ¾Æ´Ô, »ó¼¼ ¹æ¹® »ý·«)")



        # ¦¡¦¡ 3. »ó¼¼ ÆäÀÌÁö º´·Ä ¼öÁý ¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡

        print(f"[»ó¼¼ ÆäÀÌÁö ¼öÁý] {len(to_detail)}°Ç º´·Ä Á¶È¸ Áß (ÃÖ´ë {DETAIL_CONCURRENCY}°³)...")
...
# ¦¡¦¡ 3.5. _raw_summary¸¦ summary¿¡ ÀÓ½Ã ÇÒ´ç (ÇÊÅÍ¿ë) ¦¡¦¡
    ...
    print(f"\n[ÃÖÁ¾ ÇÊÅÍ] {before}°Ç ¡æ {len(all_results)}°Ç")
    ...
    if before_narrow != len(all_results):
        print(f"[ÇÊÅÍ] ÄÜÅÙÃ÷ ÇÊÅÍ: {before_narrow}°Ç ¡æ {len(all_results)}°Ç")

    # ¦¡¦¡ Claude ¿ä¾à (ÃÖÁ¾ È®Á¤µÈ ÀÇ¾È¸¸) ¦¡¦¡
    print(f"\n[Claude ¿ä¾à] {len(all_results)}°Ç ¿ä¾à »ý¼º Áß...")
    for bill in all_results:
        raw = bill.pop("_raw_summary", "")
        if raw:
            print(f"  ¡æ {bill.get('bill_name','')[:40]} ¿ä¾à Áß...")
    ...
    for bill in all_results:
        ...
        if _is_priority(bill):
            ...
            bill["priority_mark"] = "¡Ú"

            if not bill["bill_name"].startswith("¡Ú"):

                bill["bill_name"] = "¡Ú " + bill["bill_name"]
    ...
    # ¦¡¦¡ Á¤·Ä: ¨ç ¿ì¼±¼øÀ§ ÀÇ¿ø ¨è »óÅÂº¯°æ ÀÇ¾È ¨é ÃÖ½Å ¹ßÀÇÀÏ ¼ø ¦¡¦¡¦¡¦¡¦¡¦¡

    all_results.sort(

        key=lambda r: (

            0 if r.get("priority_mark") == "¡Ú" else 1,
    ...
    # 1¡¤2¼øÀ§(¿ì¼±ÀÇ¿ø, »óÅÂº¯°æ)´Â ¿À¸§Â÷¼ø(0ÀÌ ¾Õ), ³¯Â¥´Â ³»¸²Â÷¼ø ¡æ µÎ ´Ü°è Á¤·Ä
    ...
    print(f"  ÀüÃ¼: {len(all_results)}°Ç  (ÁÖ¿äÀÇ¿ø ¡Ú {priority_count}°Ç, ÀÇ°á°á°úº¯µ¿ {changed_count}°Ç)")



    # ÀÇ°á°á°ú º¯µ¿ ÀÇ¾È ¸ñ·Ï Ãâ·Â
    ...
    if changed_count:
        ...
        for r in all_results:
            ...
            if r.get("status_changed"):
                ...
                mark = "¡Ú " if r.get("is_key_member") else ""

...
if st.session_state.phase == "SELECT":
    ...
    for i, r in enumerate(asm_raw):
        ...
        if st.checkbox("이 의안 포함하기", key=f"ca_{i}"):
            sel_a.append(r)
        st.caption(f"📝 의안요약: {r.get('summary', '요약 정보가 없습니다.')}")
        st.write("")
...
# [B] 보고서 화면 (에러 지점 복구 완료)
    ...
    if st.session_state.sel_s:
        ...
        for r in st.session_state.sel_s:
            link = r.get('link', '#')
            html += f'<div style="background:#fff; border:1px solid #E2E8F0; border-left:6px solid #28A745; padding:12px 15px; margin-bottom:8
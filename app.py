...
# 주소 보정 함수 (외부 링크가 Streamlit 내부로 연결되는 것 방지)
def fix_url(url):
...
# [B] 보고서 화면
    ...
    for icon, label, val, bg in [("📋","의안",len(st.session_state.sel_a),"#EBF1F9"), ("📅","일정",len(st.session_state.sel_s),"#E8F5E9"), ("📰","뉴스",len(st.session_state.sel_n),"#FDECEA"), ("📊","전체",len(st.session_state.sel_a)+len(st.session_state.sel_s)+len(st.session_state.sel_n),"#F3F4F6")]:
        ...
        html += f'<div style="font-size:20px;">{icon}</div><div style="font-size:11px; font-weight:700;">{label}</div><div style="font-size:24px; font-weight:800;">{val}</div></div>'
    html += '</div>'
    ...
    if st.session_state.sel_a:
        ...
        for r in st.session_state.sel_a:
            link = fix_url(r.get('link', r.get('bill_link', '#')))
            html += '<div style="background:#fff; border:1px solid #E2E8F0; border-left:66px solid #1B3A6B; padding:15px; border-radius:12px; margin-bottom:10px; -webkit-print-color-adjust:exact;">'
            ...
            html += '<div style="background:#1B3A6B; color:#fff; padding:2px 10px; border-radius:12px; font-size:10px; -webkit-print-color-adjust:exact;">접수</div></div>'
            html += '<div style="background:#FFF9E6; border:1px solid #FFD966; color:#856404; padding:3px 10px; border-radius:5px; font-size:11px; font-weight:700; margin-bottom:8px; display:inline-block; -webkit-print-color-adjust:exact;">입법예고 진행중</div>'
    ...
    if st.session_state.sel_s:
        ...
        for r in st.session_state.sel_s:
            link = fix_url(r.get('link', '#'))
            html += '<div style="background:#fff; border:1px solid #E2E8F0; border-left:6px solid #28A745; padding:12px 15px; margin-bottom:8px; display:flex; justify-content:space-between; align-items:center; -webkit-print-color-adjust:exact;">'
            html += f'<div><a href="{link}" target="_blank" style="text-decoration:none; font-size:13px; font-weight:800; color:#333;">{escape(r.get("title",""))} 🔗</a>'
            html += '<div style="margin-top:3px;"><span style="background:#E8F5E9; color:#1B5E20; padding:1px 6px; border-radius:4px; font-size:10px; font-weight:700; -webkit-print-color-adjust:exact;">토론회</span></div></div>'
    ...
    if st.session_state.sel_n:
        ...
        for r in st.session_state.sel_n:
            ...
            c_hex = {"중증응급":"#800000", "중증외상":"#6F42C1"}.get(kw, "#DC3545")
            html += '<div style="background:#fff; border:1px solid #E2E8F0; border-left:6px solid #DC3545; padding:12px 15px; margin-bottom:10px; display:flex; justify-content:space-between; align-items:center; -webkit-print-color-adjust:exact;">'
            html += f'<div><a href="{link}" target="_blank" style="text-decoration:none; font-size:13px; font-weight:800; color:#1B3A6B;">{escape(r.get("title",""))} 🔗</a>'
            html += f'<div style="font-size:10px; color:#777; margin-top:3px;">{escape(r.get("source",""))
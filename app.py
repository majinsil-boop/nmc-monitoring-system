...
if st.session_state.phase == "SELECT":
    ...
    for i, r in enumerate(asm_raw):
        ...
        st.markdown(f"**{r.get('bill_name','')}** "
                    f"<a href='{link}' target='_blank'>🔗 [원문보기]</a>", 
    ...
    for i, r in enumerate(news_raw):
        ...
        st.markdown(f"📰 [{r.get('source','')}] **{r.get('title','')}** "
                    f"<a href='{link}' target='_blank'>🔗 [기사보기]</a>", 
                    unsafe_allow_html=True)
...
# [B] 보고서 화면
    ...
    html += '<div style="font-size:22px; font-weight:800;">응급의료 동향 모니터링</div></div>'
    html += f'<div style="text-align:right;"><div style="font-size:18px; font-weight:80
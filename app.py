...
def render_card(icon, label, value, text_color, bg_color, border_color):
    ...
    <div style="{style}">
        ...
        <div style="font-size:12px; font-weight:700; color:{text_color}; opacity:0.8;">{escape(label)}</div>
    </div>'''
...
if st.button("✨ 선택한 항목으로 보고서 최종 발행", use_container_width=True):
    ...
    if sel_a:
        ...
        for r in sel_a:
            ...
            b_notice = escape(r.get('legislative_notice','입법예고 진행중'))
            body_html += f'''<div style="background:#fff; border-radius:20px; border:1px solid #E2E8F0; padding:25px; margin-bottom:20px; border-left:6px solid #3B82F6; box-shadow:0 4
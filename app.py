...
def render_card(icon, label, value, text_color, bg_color, border_color):
    style = (f"flex:1; background:{bg_color}; border-radius:15px; border-top:5px solid {border_color}; "
             f"padding:20px 10px; text-align:center; box-shadow:0 2px 8px rgba(0,0,0,0.05);")
...
if st.button("✨ 선택한 항목으로 보고서 최종 발행", use_container_width=True):
    ...
    if sel_a:
        ...
        for r in sel_a:
            ...
            body_html += '<div style="display:flex; justify-content:space-between; margin-bottom:12px;">'
            body_html += '<div style="font-size:16px; font-weight:800; color:#1B3A6B;">' + escape(r.get('bill_name','')) + '</div>'
            body_html += '<div style="background:#1B3A6B; color:#fff; padding:3px 12px; border-radius:15px; font-size:11px; font-weight:700;">' + escape(r.get('status','접수')) + '</div>'
            body_html += '</div>'
            body_html += '<div style="background:#FFF9E6; border:1px solid #FFD966; color:#856404; padding:5px 12px; border-radius:5px; font-size:12px; font-weight:700; margin-bottom:15px;">' + escape(r.get('legislative_notice','입법예고 진행중')) + '</div>'
            body_html += '<div style="font-size:13px; color:#444; line-height:1.7; border-top:1px solid #F1F3F5; padding-top:15px; text-align:justify;">' + escape(r.get('summary','')) + '</div>'
            body_html += '</div>'
    ...
    if sel_s:
        ...
        for r in sel_s:
            ...
            body_html += '<div><div style="font-size:15px; font-weight:800; color:#111; margin-bottom:8px;">' + escape(r.get('title','')) + '</div>'
            body_html += '<div style="font-size:12px; color:#777;">' + escape(r.get('source','국회')) + '</div></div>'
            body_html += '<div style="text-align:right;"><div style="background:#28A745; color:#fff; padding:3px 15px; border-radius:15px; font-size:11px; font-weight:700;">예정</div>'
            body_html += '<div style="font-size:13px; font-weight:800; color:#333; margin-top:5px;">' + escape(r.get('date','')) + '</div></div>'
            body_html += '</div>'
    ...
    if sel_n:
        ...
        for r in sel_n:
            ...
            body_html += '<div><div style="font-size:14px; font-weight:700; color:#1B3A6B;">' + escape(r.get('title','')) + '</div>'
            body_html += '<div style="font-size:12px; color:#777; margin-top:5px;">' + escape(r.get('source','')) + ' | ' + escape(r.get('date','')) + '</div></div>'
            body_html += '<div style="background:#F8D7DA; color:#721C24; padding:4px 12px; border-radius:15px; font-size:11px; font-weight:700;">' + escape(r.get('keyword','기사')) + '</div>'
            body_html += '</div>'

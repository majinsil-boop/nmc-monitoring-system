...
# ══════════════════════════════════════════════════════════════════════════════
_KW_BORDER = {
    "중증응급": "#f4a8a8", "중증외상": "#f0b8b0", "응급의료": "#f0aaaa",
    "응급실": "#f4a8a8", "응급실 뺑뺑이": "#f4a8a8", "닥터헬기": "#f0b8b0",
    "상급종합병원": "#ecc0b8", "필수의료": "#f0b8b0",
}
_KW_BADGE_BG = {
    "중증응급": "#8B0000", "중증외상": "#6C3483", "응급의료": "#1B3A6B",
    "응급실": "#C0392B", "응급실 뺑뺑이": "#C0392B", "닥터헬기": "#1B3A6B",
    "상급종합병원": "#A52A2A", "필수의료": "#1B3A6B",
}
...
# HTML 빌더
# ══════════════════════════════════════════════════════════════════════════════
# CSS를 별도 문자열로 분리 (f-string 중괄호 충돌 방지)
_HTML_CSS = """
<style>
...
def build_html(sel_a, sel_s, sel_n, today) -> str:
    ...
    cards_html = (
        ...
        f'<div style="font-size:24px;font-weight:900;color:#0d2a5e;line-height:1.1">{na}</div>'
        '<div style="font-size:9.5px;color:#0d2a5e;opacity:.7;margin-top:3px;font-weight:600">계류 의안</div></div>'
        ...
        '<div style="font-size:15px;margin-bottom:3px">📅</div>'
        f'<div style="font-size:24px;font-weight:900;color:#0d4a22;line-height:1.1">{ns}</div>'
        '<div style="font-size:9.5px;color:#0d4a22;opacity:.7;margin-top:3px;font-weight:600">예정 일정</div></div>'
        ...
        '<div style="font-size:15px;margin-bottom:3px">📰</div>'
        f'<div style="font-size:24px;font-weight:900;color:#6b0000;line-height:1.1">{nn}</div>'
        '<div style="font-size:9.5px;color:#6b0000;opacity:.7;margin-top:3px;font-weight:600">언론 기사</div></div>'
        ...
        '<div style="font-size:15px;margin-bottom:3px">📊</div>'
        f'<div style="font-size:24px;font-weight:900;color:#2a2a2a;line-height:1.1">{total}</div>'
        '<div style="font-size:9.5px;color:#2a2a2a;opacity:.7;margin-top:3px;font-weight:600">전체</div></div>'
    )

    def sec_header(num, title, count):
        return (
            '<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">'
            f'<span style="display:inline-flex;align-items:center;justify-content:center;width:24px;height:24px;background:#0d2a5e;color:#fff;border-radius:50%;font-size:13px;font-weight:900;flex-shrink:0;line-height:1">{num}</span>'
            f'<span style="font-size:13px;font-weight:800;color:#0d2a5e;flex:1;letter-spacing:.1px">{title}</span>'
            f'<span style="background:#0d2a5e;color:#fff;padding:2px 11px;border-radius:20px;font-size:9px;font-weight:700">총 {count}건</span>'
            '</div>'
        )

    # ── 의안 ──
    asm_items = _dedup_assembly(sel_a)
    asm_html = '<div style="margin-bottom:16px;padding:0 8px">' + sec_header(1, "의안 현황", len(asm_items))
    if not asm_items:
        ...
        for r in asm_items:
            ...
            tags = ""
            if kw:
                tags += f'<span style="background:#dce4f5;color:#1B3A6B;padding:1px 7px;border-radius:4px;font-size:9px;font-weight:600">{escape(kw)}</span> '
            if status:
                tags += f'<span style="background:#f0f0f0;color:#555;padding:1px 7px;border-radius:4px;font-size:9px;font-weight:600">{escape(status)}</span> '
            ...
            if summ:
                summ_html = (
                    '<div style="font-size:10px;color:#555;line-height:1.5;margin-top:5px;padding:5px 8px;background:#f8f9fa;border-radius:4px">'
                    + escape(summ[:200]) + ("…" if len(summ) > 200 else "") + '</div>'
                )
            ...
            asm_html += (
                ...
                f'<span style="background:{bc};color:#fff;padding:1px 7px;border-radius:20px;font-size:9px;font-weight:700;flex-shrink:0">{lvl}</span>'
                '<div style="flex:1;font-size:11px;font-weight:700;color:#0d2a5e;line-height:1.4">'
                f'<a href="{escape(url)}" style="color:#0d2a5e;text-decoration:none">{escape(name)}</a>'
                '</div></div>'
                f'<div style="margin-bottom:4px">{tags}</div>'
                + summ_html +
                f'<div style="font-size:9.5px;color:#888;margin-top:5px">발의: {escape(date)}</div>'
                '</div>'
            )
    asm_html += "</div>"

    # ── 일정 ──
    sch_html = '<div style="margin-bottom:16px;padding:0 8px">' + sec_header(2, "주요 일정", len(sel_s))
    if not sel_s:
        ...
        for r in sel_s:
            ...
            sch_html += (
                ...
                f'<a href="{escape(url)}" style="color:#0d3a1e;text-decoration:none">{escape(title)}</a>'
                '</div>'
                '<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">'
                f'<span style="background:#d4f0dd;color:#155724;padding:1px 7px;border-radius:4px;font-size:9px;font-weight:600">{escape(etype)}</span>'
                f'<span style="font-size:9.5px;color:#666">{escape(source)}</span>'
                '</div></div>'
                ...
                f'<div style="font-size:10.5px;font-weight:700;color:#1a4a2a">{escape(date)}</div>'
                '</div></div>'
            )
    sch_html += "</div>"
    ...
    kw_tags = " ".join(
        f'<span style="display:inline-block;background:#e8e4dc;color:#555;padding:1px 9px;border-radius:20px;font-size:9px;font-weight:600">{escape(k)}</span>'
        for k in kw_set
    )
    news_html = '<div style="margin-bottom:16px;padding:0 8px">' + sec_header(3, "언론 모니터링", len(sel_n))
    if kw_tags:
    ...
    if not sel_n:
        ...
        for r in sel_n:
            ...
            news_html += (
                f'<a href="{escape(url)}" style="display:block;color:inherit;text-decoration:none;background:#fff;border-radius:10px;border-left:5px solid {border};box-shadow:0 1px 6px rgba(0,0,0,.07);padding:9px 14px;margin-bottom:8px;page-break-inside:avoid">'
                '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px">'
                '<div style="flex:1;min-width:0">'
                f'<div style="font-size:11px;font-weight:700;color:#6b0000;line-height:1.45;margin-bottom:3px">{escape(title)}</div>'
                '<div style="font-size:9.5px;color:#888">'
                f'<span style="font-weight:600;color:#555">{escape(source)}</span>'
                f'<span style="margin-left:6px">{escape(date)}</span>'
                '</div></div>'
                f'<span style="flex-shrink:0;background:{badge_bg};color:#fff;padding:2px 9px;border-radius:20px;font-size:9px;font-weight:700;white-space:nowrap;margin-top:2px">{escape(kw)}</span>'
                '</div></a>'
            )
    news_html += "</div>"
    ...
    html = (
        ...
        f'<div style="font-size:18px;font-weight:700;letter-spacing:.5px">{today_fmt}</div>'
        f'<div style="font-size:9px;opacity:.55;margin-top:3px">{generated} 생성</div>'
        '</div></div>'
        '<div style="padding:0 8px">'
        f'<div style="display:flex;gap:8px;margin-bottom:14px">{cards_html}</div>'
        + asm_html + sch_html + news_html +
